import {obtenerBares, obtenerFarmacias, obtenerTiendas} from './cargarDatos.js';
...
function mostrarMarcadores(sitios, icono) {
    ...
    for (const sitio of sitios) {
        ...
        marcador.bindPopup(`<h1>${sitio.nombre}</h1><p>URL: <a href="${sitio.url}">Visítanos</a>`);
        marcador.addTo(mapa);
    }
...
function mostrarSitios() {
    ...
    switch (tipoSitio.value) {
        case "0":
            // Debemos pasar la función que queremos que se ejecute cuando fetch devuelva los datos 
            document.getElementById("mensaje").style.display = "block";
            obtenerTiendas(mostrarMarcadores);
            break;
        case "1":
            // Debemos pasar la función que queremos que se ejecute cuando fetch devuelva los datos 
            document.getElementById("mensaje").style.display = "block";
            obtenerBares(mostrarMarcadores);
            break;
        case "2":
            // Debemos pasar la función que queremos que se ejecute cuando fetch devuelva los datos 
            document.getElementById("mensaje").style.display = "block";
            obtenerFarmacias(mostrarMarcadores);
            break;
        case "-1":
            eliminarMarcadores();
    }
